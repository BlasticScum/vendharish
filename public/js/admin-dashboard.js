document.addEventListener('DOMContentLoaded', () => {
    ...
    // Add Razorpay toggle handler
    document.getElementById('razorpayToggle')?.addEventListener('change', async (e) => {
        try {
            const response = await fetch('http://localhost:3000/api/settings/razorpay', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${localStorage.getItem('adminToken')}`
            ...
            if (!response.ok) {
                throw new Error('Failed to update Razorpay status');
            }
        } catch (error) {
            console.error('Error toggling Razorpay:', error);
            e.target.checked = !e.target.checked;
            alert('Failed to update Razorpay status');
        }
    });

    // Add news ticker toggle handler
...
async function loadSystemSettings() {
    try {
        const [settingsResponse, razorpayResponse] = await Promise.all([
            fetch('http://localhost:3000/admin/system-settings', {
                headers: {
                    'Authorization': `Bearer ${localStorage.getItem('adminToken')}`
                }
            }),
            fetch('http://localhost:3000/api/settings/razorpay', {
                headers: {
                    'Authorization': `Bearer ${localStorage.getItem('adminToken')}`
                }
            })
        ...
        const razorpayStatus = await razorpayResponse.json();
        ...
        document.getElementById('razorpayToggle').checked = razorpayStatus.enabled;
    } catch (error) {
...
// Add this function to destroy existing charts
