    ...
    try {
        ...
        const emailContent = `
            ...
            <div style="margin: 20px 0;">
                ...
                <p>Verify your ticket at: ${getCallbackUrl(`/verify-ticket/${bookingData.id}`)}</p>
        ...
        logger.info('Ticket email sent successfully', { bookingId: bookingData.id });
    } catch (error) {
...
app.post("/book", async (req, res) => {
    ...
    jwt.verify(token, JWT_SECRET, async (err, decoded) => {
        ...
        try {
            ...
            const settingsQuery = 'SELECT value FROM settings WHERE key_name = "razorpay_enabled"';
            const [settingsResults] = await connection.query(settingsQuery);
            const razorpayEnabled = settingsResults[0]?.value === 'true';

            // Create booking with appropriate status
            const bookingStatus = razorpayEnabled ? 'pending' : 'confirmed';
            ...
            connection.query(slotQuery, [timeSlot], async (err, slotResults) => {
                ...
                connection.query(
                    bookingQuery,
                    [userId, slotId, visitDate, numTickets, adultTickets, childTickets, seniorTickets, bookingStatus],
                    async (err, result) => {
                        if (err) {
                        ...
                        if (!razorpayEnabled) {
                            // Send confirmation email directly for free tickets
                            try {
...
// Add these new endpoints for Razorpay settings

// Get Razorpay status
app.get('/api/settings/razorpay', async (req, res) => {
    try {
        const query = 'SELECT value FROM settings WHERE key_name = "razorpay_enabled"';
        connection.query(query, (err, results) => {
            if (err) {
                logger.error('Error fetching Razorpay status:', err);
                return res.status(500).json({ error: "Database error" });
            }
            const enabled = results[0]?.value === 'true';
            res.json({ enabled });
        });
    } catch (error) {
...
// Update Razorpay status (admin only)
app.post('/api/settings/razorpay', async (req, res) => {
    const authHeader = req.headers.authorization;
    if (!authHeader || !authHeader.startsWith('Bearer ')) {
        return res.status(401).json({ error: "Unauthorized" });
    }
    ...
    try {
        const { enabled } = req.body;
        const query = 'UPDATE settings SET value = ? WHERE key_name = "razorpay_enabled"';
        connection.query(query, [enabled ? 'true' : 'false'], (err) => {
            if (err) {
                logger.error('Error updating Razorpay status:', err);
                return res.status(500).json({ error: "Database error" });
            }
...
// News ticker status endpoint
app.get('/admin/news-ticker-status', async (req, res) => {
    try {
        const query = 'SELECT value FROM settings WHERE key_name = "news_ticker_enabled"';
        connection.query(query, (err, results) => {
            if (err) {
                logger.error('Error fetching news ticker status:', err);
                return res.status(500).json({ error: "Database error" });
            }
            const enabled = results[0]?.value === 'true';
            res.json({ enabled });
...
app.post('/admin/news-ticker-status', async (req, res) => {
    const { enabled } = req.body;
    try {
        const query = 'UPDATE settings SET value = ? WHERE key_name = "news_ticker_enabled"';
        connection.query(query, [enabled ? 'true' : 'false'], (err) => {
            if (err) {
                logger.error('Error updating news ticker status:', err);
                return res.status(500).json({ error: "Database error" });
            }
            res.json({ success: true, enabled });
        });
